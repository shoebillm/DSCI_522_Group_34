---
title: "Null hypothesis test for the counts of graffiti in two areas of Vancouver"
...
Here we use the simulated method to test whether the null hypothesis stands: "the number of graffiti of Vancouver's downtown area is the same as the number of graffiti of Vancouver's Strathcona area".
...
Here we chose the estimator as median. The reason is that the count of graffiti per location in each area is not normal distributed and has outliers according to the population distribution, see the figure below. And median is less sensitive to the skewness and outliers.
...
As for the test flavor, we chose the permutation test.A permutation test computes the sampling distribution for any test statistic, under the ‘strong null hypothesis’ that a set of genetic variants has absolutely no effect on the outcome.

As for the test statistic, we use delta represent the difference of the median values of the counts of graffiti in each area.
...
## Data
The data set that was used in this milestone 2 project is of information 
...
The R and Python programming languages [@R; @Python] and the following R and Python packages were used to perform the analysis: caret [@caret], docopt [@docopt], knitr [@knitr], tidyverse [@tidyverse],os [@Python]. The code used to perform the analysis and create this report can be found here: https://github.com/UBC-MDS/DSCI_522_Group_34/blob/main/src/analysis.R.
...
Carrying out Mood test, we got p-value is 1, which is larger than 0.05. We don’t have enough confidence to reject the null hypotheses. It indicated that there is no statistically significant difference between the median of counts of graffiti in the 2 areas in Vancouver.
